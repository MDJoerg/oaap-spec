...
**Since 0.3.4, `broker` also carries the event relay** of
`oaap.data.twin` 0.3 (§2.13 there): `oaap node add-profile broker`
starts `broker` and `relay` together, `remove-profile` stops both — the
relay's watermark stays in each tenant schema, so events recorded
meanwhile are published once the profile is back. Its secret,
`BROKER_RELAY_KEY`, is generated by the installer and, on a node that
reaches this version by updating, by the migration step — on every
node, whether or not it carries `broker`, like
`STORE_SUPERUSER_PASSWORD` before it. Every compose call that
(re)creates the broker, the update's own included, MUST name the
...
21. **`broker` carries the relay** (0.3.4): adding the profile starts
    `relay` beside `broker`, removing it stops both; a node updated to
    this version has `BROKER_RELAY_KEY` afterwards whether or not it
    carries `broker`; and an update of a node holding `broker` AND
    `exposed` leaves port 1883 published.
...
## Deutsche Zusammenfassung (Relais am Profil `broker`, v0.3.4)

**`broker` bringt jetzt das Ereignis-Relais mit** (`oaap.data.twin`
0.3): `oaap node add-profile broker` startet Broker und Relais
gemeinsam, `remove-profile` stoppt beide. Der Stand, bis wohin
veröffentlicht wurde, liegt in jedem Mandantenschema — was in der
Zwischenzeit geschrieben wird, geht raus, sobald das Profil wieder da
ist.

**Das Geheimnis des Relais** (`BROKER_RELAY_KEY`) entsteht bei der
Installation bzw. beim Update — auf jedem Knoten, auch ohne `broker`,
damit es am Tag des Profilwechsels schon da ist.

**Ein Fund beim Bauen, gleich behoben:** `oaap update` hat den Broker
bisher ohne die Zusatzdatei für den Geräte-Port neu erzeugt. Auf einem
Knoten mit `broker` **und** `exposed` wäre Port 1883 nach jedem Update
zu gewesen. Noch trägt kein Knoten beide Profile.
