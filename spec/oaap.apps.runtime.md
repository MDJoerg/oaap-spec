...
### 2.4.4 Tenancy of an instance (0.2.18)
...
the instance exists, and whoever issues it chooses the tenant then.

An instance's tenant is decided once, when it is first installed, and a
**redeploy can never move it**: the value already on the record wins
over anything a request, a permit or a caller supplies. A redeploy that
could change the tenant is a redeploy that could hand one customer's
instance to another.
...
- **Access.** Every authenticated route of an instance is verified
  ...
  reached. Routes the app declares public are unaffected — there is no
  session there to place.
- **Administration.** A `tenant_admin` may install, remove, configure
  and issue tokens for the instances of their own tenant, and sees no
  other tenant's instances at all.
